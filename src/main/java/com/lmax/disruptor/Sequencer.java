 ...
 * 其中一个比较重要的设计是关于**GatingSequence的设计，Sequencer 是 Disruptor 的真正核心。
...
public interface Sequencer extends Cursored, Sequenced
{
     ...
     */
    long INITIAL_CURSOR_VALUE = -1L; //序列初始值 -1
     ...
     */
    void claim(long sequence); //声明一个序列，这个方法在初始化RingBuffer的时候被调用。
     ...
     */
    boolean isAvailable(long sequence); //判断一个序列是否被发布，并且发布到序列上的事件是可处理的。非阻塞方法。
     ...
     */
    boolean removeGatingSequence(Sequence sequence); //移除控制序列。
     ...
     * 获取RingBuffer上安全使用的最大的序列值,这个调用可能需要序列上从nextSequence到availableSequence之间的值。
